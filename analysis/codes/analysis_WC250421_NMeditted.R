...
# --------------------------------------------------------------- #
{
  ...
  tandemSpeed = 1.213  # NM: where does this value come from? This sets a motion threshold for detecting tandem running; summary(df$f_spe) 1st quarter result
  ...
  # NM: Consider clarifying the definition of "tandem running" explicitly in the methods section or supplementary text.
  # NM: Also, note that labeling non-tandem periods as “separation” may be misleading, as individuals might still be near each other but not actively moving.
  #WC: I have a "Close" parameter to tell if that is the case
  #Tandem Running: Tandem running in termites is a coordinated behavior in which one individual (the leader) walks ahead and is closely followed by another individual (the follower), maintaining continuous physical or near-physical contact, often via antennal or body-tip touching. 
} 
# --------------------------------------------------------------- #
...
# --------------------------------------------------------------- #
{
  ...
  # df_sepfin$event = 1
  # df_sepvid$event = 1
  #WC: Make sure Censor/Events line up with what logically makes sense, note to self
